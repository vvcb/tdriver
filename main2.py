    ...
    for aFile in filenames[0:10]:
        ...
        try:
            ...
            if len(result) == 12:
                ...
                pdfname = result + '.pdf'
                pdfpath = url2pathname(os.path.join(p_files,'pdf',pdfname))
                ...
                tmp_good.append([aFile, pdfname, result])
                #p_good_files = url2pathname(os.path.join(p_file,'good'))
                #shutil.move(docname,p_good_files)
            else:
                docpath = url2pathname(os.path.join(p_files,'bad',result + '-'+ aFile))
                doc.SaveAs(docpath)
                tmp_bad.append([aFile,result])
                #shutil.move(docname,docpath)
            ...
            tmp_bad.append([aFile,result])
            #try:
            #    p_ugly = os.path.join(p_files,'ugly')
            #    shutil.move(docname,p_ugly)
            #except:
            #    continue
